...
contract DecentralizedFundrasingPlatform {
        ...
        afterDeadline(_compaignId)
    {
        ...
        if(block.timestamp >= compaign.deadline && compaign.currentAmount >= compaign.targetAmount){
            ...
            (bool success,) = payable(compaign.creator).call{ value: balance }("");
        ...
        afterDeadline(_campaignId) 
    {
        ...
        for(uint i = 0; i < length; i++){
            ...
            (bool success,)=payable(compaignIdToCompaign[_campaignId].contributors[i]).call{value: amount}("");
    ...
    function getAmountReceived(uint _campaignId) public view returns(uint){
        return compaignIdToCompaign[_campaignId].currentAmount;
    }
}